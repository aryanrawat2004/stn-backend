  ...
  return {
    [base]: {
      get: {
        ...
        responses: {
          "200": { description: "List returned", content: { "application/json": { schema: listResponse } } },
        },
      },
      post: {
        tags: [tag],
        ...
        responses: {
          "201": { description: "Created", content: { "application/json": { schema: singleResponse } } },
          "400": errorResponse,
        },
      },
    },
    [`${base}/{id}`]: {
      get: {
        ...
        responses: {
          "200": { description: "Found", content: { "application/json": { schema: singleResponse } } },
          "404": errorResponse,
        },
      },
      put: {
        ...
        responses: {
          "200": { description: "Updated", content: { "application/json": { schema: singleResponse } } },
          "400": errorResponse,
          "404": errorResponse,
        },
      },
...
const paths: Record<string, any> = {
  ...
  "/api/admin/jobs/{id}/featured": {
    patch: {
      tags: ["Jobs"],
      summary: "Toggle featured job",
      parameters: idParameter,
      requestBody: { required: true, content: { "application/json": { schema: { type: "object", properties: { featured: { type: "boolean" } } } } } },
      responses: { "200": { description: "Updated" }, "404": errorResponse },
    },
  },
  "/api/admin/candidates/{id}/verify": { patch: booleanOperation("Candidates", "verified") },
  "/api/admin/candidates/{id}/status": { patch: statusOperation("Candidates", ["Active", "Suspended"]) },
  "/api/admin/employers/{id}/verify": { patch: booleanOperation("Employers", "verified") },
  "/api/admin/employers/{id}/status": { patch: statusOperation("Employers", ["Approved", "Pending Verification", "Suspended"]) },
  "/api/admin/companies/{id}/verify": { patch: booleanOperation("Companies", "verified") },
  "/api/admin/companies/{id}/status": { patch: statusOperation("Companies", ["Active", "Pending", "Suspended"]) },
  "/api/admin/categories/{id}/status": { patch: statusOperation("Categories", ["Active", "Inactive"]) },
  "/api/admin/ambassadors/{id}/status": { patch: statusOperation("Ambassadors", ["Active", "Pending", "Suspended"]) },
  "/api/admin/applications/{id}/status": { patch: statusOperation("Applications", ["Applied", "Screening", "Interview", "Offer", "Joined", "Rejected"]) },
...
const options: swaggerJsdoc.Options = {
  definition: {
    ...
    info: {
      ...
      version: "2.0.0",
      description: "Complete admin API for SolarNaukri jobs, talent, employers, companies, categories, ambassadors, applications, analytics and verification.",
    },
...
function booleanOperation(tag: string, field: string) {
  return {
    ...
    parameters: idParameter,
    requestBody: {
      content: { "application/json": { schema: { type: "object", properties: { [field]: { type: "boolean" } } } } },
    },
    responses: { "200": { description: "Updated" }, "404": errorResponse },
  };
}
...
function statusOperation(tag: string, allowed: string[]) {
  return {
    ...
    requestBody: {
      required: true,
      content: { "application/json": { schema: { type: "object", required: ["status"], properties: { status: { type: "string", enum: allowed } } } } },
    },
